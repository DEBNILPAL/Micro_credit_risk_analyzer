...
const CreditReportDashboard: React.FC<CreditReportDashboardProps> = ({ riskScore }) => {
  ...
  const ScoreBreakdown = () => {
    const totalWeight = Object.values(riskScore.score_components || {}).reduce((sum, comp) => sum + comp.weight, 0);
    return (
      <div className="bg-white rounded-2xl p-6 shadow-soft border border-slate-200">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="text-center">
          <div className="text-2xl font-bold text-primary-600">
            ₹{(riskScore.recommended_emi || 0).toLocaleString()}
          </div>
  ...
  const ImprovementSuggestions = () => (
    <div className="bg-white rounded-2xl p-6 shadow-soft border border-slate-200">
      ...
      <div className="space-y-4">
        {(riskScore.financial_coaching || []).map((coaching, index) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-slate-700">
              Strong payment history ({Math.round(riskScore.score_components?.payment_history?.score || 0)}%)
            </span>
          </div>
          <div className="flex items-center space-x-2">
