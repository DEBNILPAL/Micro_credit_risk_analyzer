...
export class RealisticCreditEngine {
  ...
  static calculateCreditScore(userData: UserData): RiskScore {
    ...
    return {
      ...
      interest_rate: this.calculateInterestRate(creditScore, defaultProbability),
  ...
  static calculateInterestRate(creditScore: number, defaultProbability: number): number {
    // Base rate starts at 12% and adjusts based on risk
    let baseRate = 12.0;
    
    // Credit score adjustment
    if (creditScore >= 750) baseRate -= 2.0;
    else if (creditScore >= 700) baseRate -= 1.5;
    else if (creditScore >= 650) baseRate -= 1.0;
    else if (creditScore >= 600) baseRate -= 0.5;
    else if (creditScore < 550) baseRate += 2.0;
    
    // Default probability adjustment
    if (defaultProbability > 30) baseRate += 3.0;
    else if (defaultProbability > 20) baseRate += 2.0;
    else if (defaultProbability > 10) baseRate += 1.0;
    else if (defaultProbability < 5) baseRate -= 1.0;
    
    // Ensure rate stays within reasonable bounds (8% - 24%)
    return Math.max(8.0, Math.min(24.0, baseRate));
  }
}
